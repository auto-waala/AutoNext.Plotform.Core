...
namespace AutoNext.Plotform.Core.API.Controllers
{
    [ApiController]
    [ApiVersion("1.0")]
    [Route("api/v{version:apiVersion}/vehicle-conditions")]
    public class VehicleConditionsController : ControllerBase
    {
        private readonly IVehicleConditionService _service;

        public VehicleConditionsController(IVehicleConditionService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(VehicleConditionCreateDto dto)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, VehicleConditionUpdateDto dto)
        {
