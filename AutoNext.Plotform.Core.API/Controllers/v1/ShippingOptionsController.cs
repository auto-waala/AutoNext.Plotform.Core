...
namespace AutoNext.Plotform.Core.API.Controllers
{
    [ApiController]
    [ApiVersion("1.0")]
    [Route("api/v{version:apiVersion}/shipping-options")]
    public class ShippingOptionsController : ControllerBase
    {
        private readonly IShippingOptionService _service;

        public ShippingOptionsController(IShippingOptionService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(ShippingOptionCreateDto dto)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, ShippingOptionUpdateDto dto)
        {
