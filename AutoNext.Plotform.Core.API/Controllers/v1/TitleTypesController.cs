...
namespace AutoNext.Plotform.Core.API.Controllers
{
    [ApiController]
    [ApiVersion("1.0")]
    [Route("api/v{version:apiVersion}/title-types")]
    public class TitleTypesController : ControllerBase
    {
        private readonly ITitleTypeService _service;

        public TitleTypesController(ITitleTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(TitleTypeCreateDto dto)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, TitleTypeUpdateDto dto)
        {
